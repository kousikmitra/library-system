...
if(isset($_GET['activate'])){
    $sql ="SELECT s_id FROM student WHERE s_id='{$_GET['activate']}'";
    $result = $conn->query($sql);
    if($result->num_rows == 1){
        $sql = "UPDATE student SET s_status = 1 WHERE s_id='{$_GET['activate']}'";
        if($conn->query($sql)){
            echo "<script>alert('Student Activated!'); window.location = './view-students-for-approval.php';</script>";
        }
    }
}
...
if(isset($_GET['block'])){
    ...
    if($result->num_rows == 1){
        ...
        if($conn->query($sql)){
            echo "<script>alert('Student Blocked!'); window.location = './view-students-for-approval.php';</script>";
        }
    }
}
...
<head>
    ...
    <title><?php echo "View Students For Approval | {$_SESSION['name']}"; ?></title>
</head>
<body>
<div class="main">
                ...
                <div class="main-menu">
                    ...
                    ?>
                            <tr>
                                ...
                                if($row['s_status'] == 0){
                                ?>
                                <td><a href="<?php echo "./view-students-for-approval.php?activate="."{$row['s_id']}"; ?>" class="btn btn-success" style="color:white">Activate</a>
